...
chrome.windows.getCurrent(function (wnd) {
    chrome.tabs.getSelected(wnd.id, function (tab) {
	  ...
	  var m3u8urldz = ('"' + requestM3u8 + '" --workDir "%USERPROFILE%/Downloads/m3u8dl" --enableDelAfterDone --disableDateInfo --noProxy --saveName "' + title + '"')
	  var m3u864jm = ('m3u8dl://' + window.btoa(unescape(encodeURIComponent(m3u8urldz))))
    const webPageDivHTML = `
        <div style="position:fixed;font-size:20px;word-break:break-all;margin:20px;
                top:100px;left:4px;z-index: 99999999999999;background: linear-gradient(to right, rgb(16 255 211), rgb(177 255 194));padding:4px;
                border: 4px solid #39b2f6;color: #000000;width: 15%;box-shadow: 4px 3px 9px #285a63;border-radius: 6px; transition: all 0.3s linear;-moz-box-shadow:2px 2px 5px #333333;-webkit-box-shadow:2px 2px 5px #333333;box-shadow: 4px 3px 9px #000000;"><div style="max-width: 300px;overflow: hidden; text-overflow: ellipsis;white-space: nowrap;">${title}</div>
            线路:${tabM3u8Count}
            <a target="_blank" href="${m3u864jm}"
                title="Click or right click to open the m3u8 URL">
                <span id="myM3u8DivId"style="color:white;cursor:pointer;font-size:14px;user-select:none;
                    background-color:#1565ac;border-radius:4px;
                    padding:5px 5px 5px 5px;float:right;margin-right:4px;" 
                title="调用N_m3u8DL-CLI下载视频">下载视频</span><script type="text/javascript">var title = document.getElementsByTagName("title")[0];document.write(title.innerHTML)</script>
            </a>&nbsp;          
        </div>
    `;
